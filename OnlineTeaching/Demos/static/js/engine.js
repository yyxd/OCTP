$(function() {
    ...
    var currentConversationTargetId = 0,
        ...
        token = "WtFYikrOGZiTmdh+kMhXKFRrODH5bZIgeWtohXIvTRIZq5UrVPwYZFmAjTDYVFkIzWHco1tTWB3X5e7d1htLewM6Pwvb1+Op",
        _html = "",
        namelist = {
            "group001": "高数讨论组",
            "group002": "软工导讨论组",
        ...
        owner = {
            id: "13211017",
            portrait: "../img/communicate/user_img.jpg",
            name: "陈煜"
        },
        //初始化登陆人员信息
        list = location.search.slice(1).split('&'),
        $scope = {};
    var conversationStr = '<li targetType="{0}" targetId="{1}" targetName="{2}"><span class="user_img"><img src={3} onerror="this.src=\'../Demos/static/img/communicate/personPhoto.png\'"/><font class="conversation_msg_num {4}">{5}</font></span><span class="conversationInfo"><p style="margin-top: 10px"><font class="user_name">{6}</font><font class="date" >{7}</font></p></span></li>';
    var historyStr = '<div class="xiaoxiti {0} user"><div class="user_img"><img onerror="this.src=\'../Demos/static/img/communicate/personPhoto.png\'" src="{1}"/></div><span>{2}</span><div class="msg"><div class="msgArrow"><img src="../Demos/static/img/communicate/{3}"> </div><span></span>{4}</div><div messageId="{5}" class="status"></div></div><div class="slice"></div>';
    var friendListStr = '<li targetType="4" targetId="{0}" targetName="{1}"><span class="user_img"><img src="../Demos/static/img/communicate/personPhoto.png"/></span> <span class="user_name">{1}</span></li>';
    var discussionStr = '<li targetId="{0}" targetName="{1}" targetType="{2}"><span class="user_img"><img src="../Demos/static/img/communicate/user.png"/></span><span class="user_name">{3}</span></li>';
    /*if (list.length == 3) {
  ...
  //未读消息数
    ...
    //初始化SDK
    RongIMClient.init("vnroth0krkjto"); //e0x9wycfx7flq z3v5yqkbv8v30
...
/*
    $.ajax({
        ...
        password: "rongyun19951213",
        data: {
            userId: owner.id,
            name: owner.name,
            //portraitUri: owner.portrait,
        },
        crossDomain : true,
        dataType: 'jsonp',
    }).done(function(data) {
